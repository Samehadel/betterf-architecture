...
- `/` lazy-loads a standalone, OnPush Angular landing page. The preserved foundation diagnostic page is `/status`; it uses the status store and relative `/api/status` client. Unknown paths redirect to `/`. No authentication guard or backend request is needed.
...
- No API client, state store, form, analytics, remote font, registration endpoint, or user persistence is required for the landing page itself. The separate `/status` diagnostic retains its route-scoped store and HTTP timeout/retry behavior.
...
The existing backend foundation is retained under `app/backend`: Spring Boot, PostgreSQL, Liquibase, and the handwritten status contract. GET `/api/status` returns the standard data/error envelope, reports database unavailability as 503, and requires no login. Readiness includes the database; liveness does not. Other application endpoints are denied by default. The initial migration only establishes migration history; no business tables are introduced. Backend checks exercise real PostgreSQL startup, migration history, security denial, outage/recovery, response wrapping, and module boundaries.
